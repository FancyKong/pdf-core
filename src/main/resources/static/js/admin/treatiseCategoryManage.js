	...
	$(document).ready(function() {
		oTable = $('#otable').DataTable(
			...
			"ajax" : function(data, callback, settings) {
				//封装请求参数
				var param = treatise_categoryManage.getQueryCondition(data);
				$.ajax({
					type : "GET",
					url : "/treatise_category/page",//TODO
					cache : false, //禁用缓存
					data : param, //传入已封装的参数
					dataType : "json",
	...
	var treatise_categoryManage = {
		...
		getQueryCondition : function(data) {
			...
			//组装查询参数
			param.fuzzySearch = treatise_categoryManage.fuzzySearch;
    ...
    $('#otable').on('click', 'a.op_delete', function (e) {
        ...
        myConfirm("你确定要删除吗?",function(){
            //向服务器提交删除请求
            var url = "/treatise_category/"+id+"/delete";
    ...
    $('#otable').on('click', 'a.op_edit', function (e) {
        ...
        var nRow = $(this).parents('tr')[0];
        var id = oTable.row(nRow).id();
        var url = "/treatise_category/" + id + "/update";
        window.open(url, "_self");
    });