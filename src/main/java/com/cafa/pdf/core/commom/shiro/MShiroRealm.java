package com.cafa.pdf.core.commom.shiro;
...
public class MShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        log.info("【ShiroRealm 权限认证】 loginType:{} ", loginType);

        if ("AUTHOR".equals(loginType)){
            Set<String> rolesName = new HashSet<>();
            rolesName.add("author");
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        String loginType = (String) SessionUtil.get("loginType");
        log.info("【ShiroRealm 登录验证】 loginType:{} ", loginType);

        if ("AUTHOR".equals(loginType)){
            //查出著作者
            Author author = authorService.findByUsername(token.getUsername());
            if (author == null) {
                throw new UnknownAccountException("不存在此账号");
            }
            if(1 != author.getActive()){
                throw new LockedAccountException("已被禁用");
            }
            return new SimpleAuthenticationInfo(
                    new ShiroUser(author.getId(), author.getUsername(), author.getNickname()),
                    author.getPassword(), getName());
        }else if("ADMIN".equals(loginType)) {
            //查出是否有此用户
            ...
            return new SimpleAuthenticationInfo(
                    new ShiroUser(user.getId(), user.getUsername(), user.getNickname()),
